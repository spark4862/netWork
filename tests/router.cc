...
class Host {
 ...
 public:
  Host(string name, const Address& my_address,
       const Address& next_hop)  // NOLINT(*-easily-swappable-*)
      : _name(std::move(name)),
  ...
  InternetDatagram send_to(const Address& destination, const uint8_t ttl = 64) {
    ...
    dgram.payload.emplace_back(
        string{"random payload: {" + to_string(random_device()()) + "}"});
...
class Network {
 private:
  ...
  static void exchange_frames(const string& x_name, AsyncNetworkInterface& x,
  ...
  static void deliver(const string& src_name, AsyncNetworkInterface& src,
  ...
  static void deliver(const string& src_name, AsyncNetworkInterface& src,
                      const string& dst1_name, AsyncNetworkInterface& dst1,
                      const string& dst2_name, AsyncNetworkInterface& dst2) {
    while (optional<EthernetFrame> frame = src.maybe_send()) {
      cerr << "Transferring frame from " << src_name << " to " << dst1_name
           << " and " << dst2_name << ": " << summary(*frame) << "\n";
 ...
 public:
  Network()
      : default_id(_router.add_interface(
            {random_router_ethernet_address(), Address{"171.67.76.46"}})),
        eth0_id(_router.add_interface(
            {random_router_ethernet_address(), Address{"10.0.0.1"}})),
        ...
        uun3_id(_router.add_interface(
            {random_router_ethernet_address(), Address{"198.178.229.1"}})),
        hs4_id(_router.add_interface(
            {random_router_ethernet_address(), Address{"143.195.0.2"}})),
        mit5_id(_router.add_interface(
            {random_router_ethernet_address(), Address{"128.30.76.255"}})) {
    _hosts.insert({"applesauce",
                   {"applesauce", Address{"10.0.0.2"}, Address{"10.0.0.1"}}});
    _hosts.insert({"default_router",
                   {"default_router", Address{"171.67.76.1"}, Address{"0"}}});
    _hosts.insert(
        {"cherrypie",
         {"cherrypie", Address{"192.168.0.2"}, Address{"192.168.0.1"}}});
    _hosts.insert(
        {"hs_router", {"hs_router", Address{"143.195.0.1"}, Address{"0"}}});
    _hosts.insert(
        {"dm42",
         {"dm42", Address{"198.178.229.42"}, Address{"198.178.229.1"}}});
    _hosts.insert(
        {"dm43",
         {"dm43", Address{"198.178.229.43"}, Address{"198.178.229.1"}}});

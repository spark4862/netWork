...
int main() {
  try {
...

    {
      ...
      test.execute(Insert{
          {0x0d, 0x0a, 0x63, 0x61, 0x0a, 0x66, 0x65, 0x20, 0x62, 0x30}, 0});
      test.execute(BytesPushed(16));
      test.execute(BytesPending(0));
      test.execute(ReadAll({0x0d, 0x0a, 0x63, 0x61, 0x0a, 0x66, 0x65, 0x20,
                            0x62, 0x30, 0x0d, 0x62, 0x00, 0x61, 0x00, 0x00}));
    }

...
int main() {
  try {
...

    {
      ...
      TCPSenderTestHarness test{
          "Don't add FIN if this would make the segment exceed the receiver's "
          "window",
          cfg};
      test.execute(Push{});
      test.execute(ExpectMessage{}
...

    {
      ...
      TCPSenderTestHarness test{
          "When filling window, treat a '0' window size as equal to '1' but "
          "don't back off RTO",
          cfg};
      test.execute(Push{});
      test.execute(ExpectMessage{}
...

    {
      ...
      TCPSenderTestHarness test{
          "Unlike a zero-size window, a full window of nonzero size should be "
          "respected",
          cfg};
      test.execute(Push{});
      test.execute(ExpectMessage{}

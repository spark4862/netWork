    ...
    const size_t random_seed)  // NOLINT(bugprone-easily-swappable-parameters)
{
  ...
  ByteStreamTestHarness bs{"stress test input=" + to_string(input_len) +
  ...
  while (expected_bytes_pushed < data.size() or
    ...
    /* write something */
    uniform_int_distribution<size_t> bytes_to_push_dist{
        0, data.size() - expected_bytes_pushed};
    ...
    expected_available_capacity -=
        min(amount_to_push, expected_available_capacity);
